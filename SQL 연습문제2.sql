...
grant all privileges on `bank`.* to 'everybody24'@'%';
grant all privileges on `bank`.* to 'Shop'@'%';
flush privileges;
...
select * from `bank_transaction` where `t_dist` <= 2 order by `t_dist`, `t_amount` desc;
